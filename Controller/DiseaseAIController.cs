...
{
    [Authorize]
    public class DiseaseAIController : BaseController
    {
        private readonly IOptions<ConfigApiValues> _configValues;
        public DiseaseAIController(IOptions<ConfigApiValues> config)
        {
        ...
        public IActionResult GetAllDiseases(PaginationRequestModel paginationModel)
        {
            try
            {
                var returnModel = ApiHelper.CallApi(_configValues.Value.ApiUrl, _configValues.Value.ApiKey, "DiseaseAI/GetAllDiseases", Method.POST, JsonConvert.SerializeObject(paginationModel), UserSession?.Token);
        ...
        public IActionResult AddUpdate(int diseaseId)
        {
            ...
            if (diseaseId != 0)
            {
                var returnModel = ApiHelper.CallApi(_configValues.Value.ApiUrl, _configValues.Value.ApiKey, "DiseaseAI/GetDiseaseById", Method.POST, JsonConvert.SerializeObject(diseaseId), UserSession?.Token);
                if (returnModel.Success)
                {
        ...
        {
            var returnModel = ApiHelper.CallApi(_configValues.Value.ApiUrl, _configValues.Value.ApiKey, "DiseaseAI/SaveDisease", Method.POST, JsonConvert.SerializeObject(model), UserSession?.Token);
            if (returnModel != null)
            {
        ...
        public IActionResult SaveRelatedVideo(DiseaseModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var returnModel = ApiHelper.CallApi(_configValues.Value.ApiUrl, _configValues.Value.ApiKey, "DiseaseAI/SaveRelatedVideo", Method.POST, JsonConvert.SerializeObject(rModel), UserSession?.Token);
                if (returnModel != null)
                {
        ...
        public IActionResult DeleteDisease(int dId)
        {
            try
            {
                var returnModel = ApiHelper.CallApi(_configValues.Value.ApiUrl, _configValues.Value.ApiKey, "DiseaseAI/DeleteDisease", Method.POST, JsonConvert.SerializeObject(dId), UserSession?.Token);
                if (returnModel != null)
                {
